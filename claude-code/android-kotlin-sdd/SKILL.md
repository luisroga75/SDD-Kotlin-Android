...
description: "Guía el ciclo SDD de apps Android con Kotlin en Claude Code: entrevista, instrucciones, constitución, specs, planes, tareas, implementación, validación y Git/GitHub. Úsala para iniciar, continuar o revisar este proceso; no para consultas aisladas de Kotlin."
---

# SDD para Android con Kotlin en Claude Code

Invocación: `/android-kotlin-sdd`. Lee CLAUDE.md y sus imports. En proyectos nuevos propone AGENTS.md común y CLAUDE.md con `@AGENTS.md`, como en el curso; no asumas lectura automática de AGENTS.md. Conserva un CLAUDE.md existente y sus reglas; si el equipo usa solo ese archivo, adapta las referencias sin imponer otro contrato duplicado. No presupongas herramientas de Codex ni concedas permisos mediante esta skill.
...
2. Lee las instrucciones CLAUDE aplicables y sus imports (incluido AGENTS si se importa), cambios locales y documentos SDD antes de escribir. Si hay código, inspecciona configuración Gradle, catálogo de versiones, manifiestos y pruebas relevantes. En una carpeta vacía puedes documentar sin crear aún una app.
...
```text
python3 <ruta-de-la-skill>/scripts/audit_sdd.py <raiz-del-proyecto> --agent claude
python3 <ruta-de-la-skill>/scripts/audit_sdd.py <raiz-del-proyecto> --agent claude --ready
```
